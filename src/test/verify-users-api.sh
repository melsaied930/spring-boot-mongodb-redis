...
BODY=$(echo "$RESPONSE" | sed '$d')
STATUS=$(echo "$RESPONSE" | tail -n1)
fail_if_not "200" "$STATUS" "GET user by ID"
fail_if_not "verifytester" "$(echo "$BODY" | jq -r '.username')" "Username check"
...
RESPONSE=$(curl -s -w "\n%{http_code}" -X PUT "$BASE_URL/$USER_ID" \
  -H "$CONTENT_TYPE" -d "$UPDATED_JSON")
BODY=$(echo "$RESPONSE" | sed '$d')
STATUS=$(echo "$RESPONSE" | tail -n1)
fail_if_not "200" "$STATUS" "Update user"
fail_if_not "updatedtester" "$(echo "$BODY" | jq -r '.username')" "Updated username check"
...
STATUS=$(echo "$RESPONSE" | tail -n1)
fail_if_not "204" "$STATUS" "Delete user"
